
# 안녕하세요! 컴퓨터 비전 연구자 김정연입니다.

저는 서울과학기술대학교 인공지능응용학과 석사 과정에 재학중이며, Surveilance video captioning,
open-vocabulary video anomaly detection, open-vocabulary semantic segmentation, action
recognition in basebal 등 폭넓은 비전 태스크의 연구를 진행해왔습니다. 또한, 야구 태그아웃 판독
시스템을 개발하여 객체 간 상호작용을 모델링하고 프레임별 중요도를 시각화하는 설명 가능한 AI를
구현한 경험과 같이 실무 프로젝트를 통해 공부한 이론을 실제 적용 가능한 솔루션으로 구현하는
경험을 쌓아왔으며 다양한 컴퓨터비전 기술을 활용해 실질적인 문제를 해결하는 것에 큰 관심을 갖고
있습니다.

---

## 🔥 Activity

- CVLab, Seoultech (컴퓨터비전 연구실 전일제 연구원) · 2024.09 ~ 2026.02 
  - Surveillance Video Captioning 연구 (연구책임자, 여대학원생 공학연구팀제) · 2025.04 ~  2025.11
  - Open-Vocabulary Video Anomaly Detection 연구 (연구책임자, 여대학원생 공학연구팀제) · 2025.04 ~ 2025.11
- CVLab, Seoultech (컴퓨터비전 연구실 학부 인턴)  2022.07 ~ 2024.08
  - Open-Vocabulary Semantic Segmentation 연구 (기초연구실 과제 참여 연구원) · 2023.09 ~  2025.03
  - Baseball Action Recognition 캡스톤 프로젝트 · 2023.09 ~ 2024.06
  - Single Image Superesolution · 2022.12 ~ 2023.06

---

## 🧠 Interests

- **Vision-language model**
- Multi-modal learning
- Surveillance video captioning & 보안 관제용 멀티모달 시스템  
- Open-vocabulary video anomaly detection  
- Open-vocabulary semantic segmentation  
- Video understanding & action recognition
- Explainable AI, 모델 해석 및 시각화  

---

## 🏆 Awards

- 2024 SeoulTech 캡스톤 디자인 경진대회 **2등**  
  - 주제 : 컴퓨터비전을 활용한 야구 태그아웃 판정 시스템 개발 
- 2023 Samsung AI Challenge: Image Quality Assessment **최우수상 (1등)**  
- 2023 전자공학회 하계학술대회 **우수 논문상**  
  - 주제 : 객체 분할을 위한 가변거리 기반의 3D LiDAR 포인트 클라우드 그룹핑
- 2022 동계 인공지능 SCI 부트캠프 **대상 (1등)**  

---

## 📚 Publications
**International**
- **Open Vocabulary Segmentation with Context-Aware Text Embedding**, Knowledge-Based Systems, submitted.  
- **Data-Efficient Electricity Consumption Forecasting with a Tabular Foundation Model**, ICTC 2025.  
- **TAVAD:Open-Vocabulary Temporal-Aware Video Anomaly Detection**, IEEE Access, in writing.  
- **Dynamic Hierarchical Clustering with Novel Metrics for Efficient Visual Document Retrieval**, IEEE Access, in writing.

**Domestic**
- **개방형 어휘 비용 집계를 활용한 비디오 이상치 탐지**, 대한전자공학회 학술대회, pp. 2986–2989, 2025.
- **객체 분할을 위한 가변거리 기반의 3D LiDAR 포인트 클라우드 그룹핑**, 전자공학회 논문지, Vol. 61, No. 4, 2024-04.
- **주파수 기반의 적응적 가중 손실을 활용한 초해상도 복원**, 대한전자공학회 학술대회, pp. 1474–1477, 2023.
   

---

## 🛠 Tech Stack

- **Languages**: Python, C++, C  
- **Frameworks**: PyTorch, TensorFlow, Keras  
- **Etc.**: Git, Linux, CUDA, LaTeX  

---

## 📫 Contact

- GitHub: [@kimjy-st](https://github.com/kimjy-st)  
- Email: hahajungyeon@gmail.com  
